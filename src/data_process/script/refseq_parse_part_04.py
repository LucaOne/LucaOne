...
def parse(dirpath, save_dir, prefix, chunk_size=1000000, split_size=4, split_idx=0):
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    err_wfp = open(os.path.join(save_dir, "err_parse_%2d.txt" % (split_idx + 1)), "w")
    ...
    total_seq_cnt = 0
    total_fea_cnt = 0
    dirpath_list = os.listdir(dirpath)
    dirpath_list = sorted(dirpath_list)
    per_num = (len(dirpath_list) + split_size - 1)//split_size
    begin = split_idx * per_num
    end = min((split_idx + 1) * per_num, len(dirpath_list))
    for dirname in dirpath_list[begin:end]:
...
'''
total_fasta_cnt: 18893447 -> 18862614
total_seq_cnt: 18893447
total_fea_cnt: 577378743
dir_num: 59556
total: 18893447
'''
parse("/mnt2/yanyan/refseq_data/",
      "/mnt3/sanyuan.hy/data/refseq/refseq_parsed_part_04/",
      "refseq_part_04",
      chunk_size=100000,
      split_size=5,
      split_idx=3)