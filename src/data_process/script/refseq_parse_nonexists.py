...
def get_not_exists_assembly(filepath):
    ...
    print("non exists assembly_accession: %d" % len(assembly_accession_set))
...
def parse(dirpath, save_dir, prefix, chunk_size=1000000):
    ...
    err_wfp = open(os.path.join(save_dir, "err_parse_nonexists.txt"), "w")
    not_exists_assembly_set = get_not_exists_assembly("./tmp_lucaone_v2_refseq_seq_info_not_exists_assembly_accession.csv")
...
parse("/mnt2/yanyan/refseq_data/",
      "/mnt3/sanyuan.hy/data/refseq/nonexists_parsed/",
      "refseq",
      chunk_size=100000)