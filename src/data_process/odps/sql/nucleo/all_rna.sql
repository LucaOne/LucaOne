...
--create time:2023-11-16 14:33:10
--********************************************************************--
...
DROP TABLE IF EXISTS luca_data2.tmp_lucaone_v2_refseq_fea_info_intron_manual_rna ;
-- 166392479->166392870
CREATE TABLE IF NOT EXISTS luca_data2.tmp_lucaone_v2_refseq_fea_info_intron_manual_rna  AS
SELECT  *
FROM    (
            SELECT  assembly_accession
                 ...
                 ,qualifiers
                 ,insert_date
            FROM    luca_data2.tmp_lucaone_v2_refseq_fea_info_rna
...
--********************************************************************--
--author:sanyuan_xhs_test
--create time:2023-11-24 00:56:54
--********************************************************************--
SET odps.sql.python.version=cp37;
...
DROP TABLE IF EXISTS tmp_lucaone_v2_refseq_nucleo_100w_split_c1_rna_${max_segment_len} ;

-- 使用gene片段分片,seq_id, feature_type, start_end, seq_len, max_len
CREATE TABLE IF NOT EXISTS tmp_lucaone_v2_refseq_nucleo_100w_split_c1_rna_${max_segment_len} AS
...
FROM    (
            ...
            FROM    (
                        ...
                        FROM    (
                                    SELECT  *
                                    FROM    luca_data2.tmp_lucaone_v2_refseq_fea_info_intron_manual_rna
                                    WHERE   seq_id IN (
...
DROP TABLE IF EXISTS tmp_lucaone_v2_refseq_nucleo_100w_split_c2_rna_${max_segment_len};

-- 无gene有feature 合并 无feature的, seq_id, molecule_type, feature_type, start_end, seq_len, max_len)


CREATE TABLE IF NOT EXISTS tmp_lucaone_v2_refseq_nucleo_100w_split_c2_rna_${max_segment_len} AS
SELECT  refseq_extract_split_info(seq_id,split_info) AS (seq_id,strand,start_p,end_p,fragment_type,gene_idx,split_type,segment_len)
FROM    (
            SELECT  seq_id
...
DROP TABLE IF EXISTS tmp_lucaone_v2_refseq_nucleo_100w_split_rna_${max_segment_len};

CREATE TABLE IF NOT EXISTS tmp_lucaone_v2_refseq_nucleo_100w_split_rna_${max_segment_len} AS
SELECT  *
FROM    (
            SELECT  *
            FROM    tmp_lucaone_v2_refseq_nucleo_100w_split_c1_rna_${max_segment_len}
            UNION ALL
            SELECT  *
            FROM    tmp_lucaone_v2_refseq_nucleo_100w_split_c2_rna_${max_segment_len}
        ) tmp
;

DROP TABLE IF EXISTS luca_data2.tmp_lucaone_v2_refseq_100w_label_detail_rna_${max_segment_len};
...
CREATE TABLE IF NOT EXISTS luca_data2.tmp_lucaone_v2_refseq_100w_label_detail_rna_${max_segment_len} AS
SELECT  seq_id
     ,gene_idx
     ,strand
     ,fragment_type
...
FROM    (
            ...
            FROM    (
                        SELECT  DISTINCT seq_id
                                       ...
                                       ,end_p AS seq_end_P
                        FROM    luca_data2.tmp_lucaone_v2_refseq_nucleo_100w_split_rna_${max_segment_len}
                    ) t1
                        LEFT JOIN (
                SELECT  seq_id
                     ,molecule_type
...
CREATE FUNCTION refseq_labels_process AS 'refseq_labels_process_udf.refseq_labels_process'
    USING 'refseq_labels_process_udf.py,gene_type_span_level_label_v2.txt,gene_taxonomy_seq_level_label_v2.txt,biopython-1.80-cp37.zip' -f ;

DROP TABLE IF EXISTS luca_data2.tmp_lucaone_v2_refseq_100w_all_detail_rna_${max_segment_len};

CREATE TABLE IF NOT EXISTS luca_data2.tmp_lucaone_v2_refseq_100w_all_detail_rna_${max_segment_len} AS
SELECT  seq_id
     ,gene_idx
     ,seq_start_p
     ,seq_end_p
...
FROM    luca_data2.tmp_lucaone_v2_refseq_100w_label_detail_rna_${max_segment_len}
GROUP BY seq_id
       ,gene_idx
       ,seq_start_p
       ,seq_end_p
...
drop table if exists luca_data2.tmp_lucaone_v2_refseq_100w_all_detail_rna_${max_segment_len}_step1;
CREATE TABLE if not exists luca_data2.tmp_lucaone_v2_refseq_100w_all_detail_rna_${max_segment_len}_step1 AS
SELECT seq_id
     ,gene_idx
     ,seq_start_p
     ,seq_end_p
...
FROM  luca_data2.tmp_lucaone_v2_refseq_100w_all_detail_rna_${max_segment_len};
...
drop table if exists luca_data2.tmp_lucaone_v2_refseq_100w_all_detail_rna_${max_segment_len}_step2;
create table luca_data2.tmp_lucaone_v2_refseq_100w_all_detail_rna_${max_segment_len}_step2 AS
select *
from (
         select * ,refseq_span_verify(seq_id, seq_end_p - seq_start_p, labels) as flag
         from luca_data2.tmp_lucaone_v2_refseq_100w_all_detail_rna_${max_segment_len}_step1
     ) tmp
where flag is null;
...
drop table if exists tmp_lucaone_v2_refseq_rna_all_detail_v2_${max_segment_len};
create table if not exists tmp_lucaone_v2_refseq_rna_all_detail_v2_${max_segment_len}
as
select t1.seq_id as seq_id, gene_idx, seq_start_p, seq_end_p, molecule_type, labels,
       SUBSTR(seq, seq_start_p + 1, seq_end_p - seq_start_p) as seq_segment
from tmp_lucaone_v2_refseq_100w_all_detail_rna_${max_segment_len}_step2 t1
         join (
...
drop table if exists tmp_lucaone_v2_refseq_rna_all_detail_v2_${max_segment_len}_verfiy;
create table if not exists tmp_lucaone_v2_refseq_rna_all_detail_v2_${max_segment_len}_verfiy
as
select *
...
select distinct concat(seq_id, "_", gene_idx, "_", seq_start_p, "_", seq_end_p, "_", molecule_type) as obj_id,
                ...
                labels as obj_label
from tmp_lucaone_v2_refseq_rna_all_detail_v2_${max_segment_len}_verfiy;
